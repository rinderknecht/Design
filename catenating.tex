...
\end{array}
\end{equation*}
It is important not to rush to write the right\hyp{}hand sides. Are
some cases left out? No, because there are exactly two arguments which
can each be either empty or not, leading to exactly \(2 \cdot 2 = 4\)
cases. Then, we write the corresponding canvas:
\begin{equation*}
\begin{array}{r@{\;}l@{\;}l}
...
defined. Therefore, what we need here is a recursive call:
\begin{equation*}
\begin{array}{r@{\;}l@{\;}l}
\fun{join}(\el,\el) & \rightarrow & \el;\\
\fun{join}(\el,\cons{y}{t}) & \rightarrow & \cons{y}{t};\\
...
right\hyp{}hand sides and wonder twofold: (1)~Are the right\hyp{}hand
sides rewritten into the expected type of value, for all function
calls?  (2)~Are the function calls being provided the expected type of
arguments?  These checks stem from the fact that some functional
languages, like \Erlang, do not include \emph{type inference} at
