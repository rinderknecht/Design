...
\(y=\word{curiosity}\) at index~\(5\), written \(\ind{y}{5,7} = x\),
meaning \(\ind{y}{5} = \ind{x}{0}\), \(\ind{y}{6} = \ind{x}{1}\) and
\(\ind{y}{7} = \ind{x}{2}\). Given two words \(p\)~and~\(t\),
...
length~\(m\) over the alphabet~\(\breve{a}\). Therefore, the average
number \(\OM{\fun{loc}}{m,n}\) of comparisons for the na\"{\i}ve
...
backtrack). Consider a partial run in \fig~\vref{fig:mp_ex}
\begin{figure}[t]
\centering
\includegraphics{mp_ex}
\caption{Morris\hyp{}Pratt algorithm at work}
\label{fig:mp_ex}
\end{figure}
where \(p\)~is not found to be a factor of a prefix of~\(t\) (\(t\) is
longer than shown). As usual, letters on a grey background correspond
to mismatches. It is clear that \(\Border{}{a} = \varepsilon\), for
all letters~\(a\).
...
= \Border{}{\ind{x}{1,n-1}},
\end{align*}
where the \((\smash{\eqn{k-1}})\) suppose \(\Border{}{\ind{x}{0,k-2}}
\cdot \ind{x}{k-1} \nprefeq \ind{x}{0,k-2}\), and
...
\fig~\vref{fig:mp_fail} called a \emph{deterministic finite automaton},
\index{word factoring!Morris-Pratt!automaton} \index{finite
  automaton!deterministic $\sim$} and it is shown in
...
\label{fig:abacabac}
\end{figure}
We will here only describe informally such automata; for a full
treatment, see \cite{VanLeeuwen_1990c, HopcroftMotwaniUllman_2003,
