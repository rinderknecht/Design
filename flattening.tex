...
\caption{Minimum costs for the catenations in \fun{flat\(_0\)/1}
\label{fig:cat_min}}
\end{figure}
...
\caption{Defining \fun{flat/1}\label{fig:flat}
...
all these inequalities, the result \(\Omega = 0 \Rightarrow L
\leqslant C\) ensues, where \(C\)~is the cost of rewriting the calls
to~\fun{cat/2}\index{cat@\fun{cat/2}}
in~\fun{flat\(_0\)/1}\index{flat0@\fun{flat$_0$/1}}. Because
\(\C{\fun{flat}}{n,\Omega,\Gamma} = (1 + n + \Omega + \Gamma) + L\)
and \(\C{\smash{\fun{flat}_0}}{n,\Omega,\Gamma} = (1 + n + \Omega +
\Gamma) + (C + \Gamma)\), the consequence is that, if there are no
empty stacks, \fun{flat/1}\index{flat@\fun{flat/1}} is faster.
...
\citep{ArtsGiesl_1996}. Again, the same order works: \(\cons{y}{t}
\succ t\) (rule~\(\delta\) and rule~\(\omega\) when \(y=\el\));
\(\cons{\cons{x}{s}}{t} \succ t\) and \(\cons{\cons{x}{s}}{t} \succ
\cons{x}{s}\) (rule~\(\gamma\)). Termination ensues.\hfill\(\Box\)
...
fails: \(\cons{\cons{x}{s}}{t} \nsucc \cons{x,s}{t} =
\cons{x}{\cons{s}{t}}\). We could try the more general \emph{proper
  subterm order}\index{induction!proper subterm order}, that is, the
strict inclusion of a term into another, but, despite \(\cons{x}{s}
\succ x\), we have \(t \nsucc \cons{s}{t}\). One way out is to define
a \emph{measure}\index{termination!measure} on the stacks
...
respect to rewriting: \(x \rightarrow y \Rightarrow \measure{x} \succ
\measure{y}\). Actually, we will only consider \emph{dependency pairs}
...
pairs are\index{flat@\fun{flat/1}} \((\fun{flat}(\cons{\el}{t}),
\fun{flat}(t))_\omega\), \((\fun{flat}(\cons{\cons{x}{s}}{t}),
\fun{flat}(\cons{x,s}{t}))_\gamma\) and \((\fun{flat}(\cons{y}{t}),
\fun{flat}(t))_\delta\), with \(y \not\in S\). We can actually drop
the function names, as all the pairs involve
...
\measure{y} := 0,\;\text{if \(y \not\in S\) or \(y=\el\)}.
\end{equation*}
We have \(\measure{\cons{\cons{x}{s}}{t}} = 3 + 4\cdot\measure{x} +
2\cdot\measure{s} + \measure{t}\) and, for the other stack,
\(\measure{\cons{x,s}{t}} = 2 + 2\cdot\measure{x} + 2\cdot\measure{s}
+ \measure{t}\). Because \(\measure{x} \in \mathbb{N}\), for
all~\(x\), we have \(\measure{\cons{\cons{x}{s}}{t}} >
