...
desirable: the binary tree should be as balanced\index{binary
  tree!balanced $\sim$} as possible and the choice of visiting the
left or right subtree should be taken only upon examining the contents
in the root, called \emph{key}\index{key}.

\begin{wrapfigure}[7]{r}[0pt]{0pt}
\centering
\includegraphics[bb=71 661 118 724]{bst_ex} %724]
\caption{}
\label{fig:bst_ex}
\end{wrapfigure}
...
\fun{bst\(_1\)/2}\index{bst1@\fun{bst\(_1\)/2}} and, in turn, one call
to \fun{cmp/3}\index{cmp@\fun{cmp/3}}; also, all the \(n+1\) external
...
\emph{aliases}\index{memory!aliasing} in patterns or, equivalently, if
the compiler can detect that the term \(\fun{bst}(x,t_1,t_2)\) can be
shared instead of being duplicated in the second rule of
\fun{mem\(_4\)/3} (here, we assume that sharing is implicit and
maximum within a rule).\index{mem4@\fun{mem\(_4\)/3}} For additional
...
deletion. It requires two kinds of internal nodes, \fun{bst/3} and
\fun{del/3}.\index{del@\fun{del/3}} This alternative design is shown
