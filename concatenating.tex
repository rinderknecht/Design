...
Let \(s\)~be a stack of \(n\)~items \([x_0,x_1,\dots,x_{n-1}]\). We
have the following:
\begin{equation*}
\begin{array}{r@{\;}c@{\;}l}
...
The key is to realise that we need to start the same process again,
...
may lead to a slower program. Moreover, the definition we derived in
tail form may be longer, as illustrated by \fun{cat\(_0\)/2}: four
...
= \len{\alpha} + \len{\delta}\). Therefore, we have the following:
\begin{align*}
\C{\fun{cat\(_0\)}}{n}
