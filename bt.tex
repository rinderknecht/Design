...
\begin{figure}
\centering
\includegraphics[bb=71 628 252 721]{bt_shape}
...
\centering
\includegraphics[bb=71 648 177 720]{leaf_tree}%723]{leaf_tree}
\caption{A leaf tree}
\label{fig:leaf_tree}
\end{wrapfigure}
...
\emph{depth}\index{binary tree!depth} of a tree, coming from graph
...
conquer} principle\index{design!big-step $\sim$}\index{divide and conquer|see{design, big-step $\sim$}} because it springs from
...
the \emph{toll}\index{toll} and the closed form and asymptotic
...
\begin{figure}[!t]
\centering
\includegraphics[bb=71 650 374 723]{pre3_abcde_0} %[... ... ... 723]
...
The cost of \(\fun{pre}_3(t)\)\index{pre3@\fun{pre\(_3\)/1}}, where
...
Let us have a look at the internal node
...
For instance, the subtree~\(t_2\) corresponds to the recursive call
...
letting the stack invariant. Finally we have the functional program in
\fig~\vref{fig:pre}.
\begin{figure}[b]
\begin{equation*}
\boxed{
...
The function \fun{npre/1}\index{npre@\fun{npre/1}} (\emph{number in
...
inference systems\index{induction!inference system}, first seen
...
lexicographic order we just defined is \((\fun{ext}(),\el)\).

The well\hyp{}founded induction principle then requires that we
establish
\begin{itemize*}
...
\label{fig:in}
\end{figure}
whose cost is the same as for preorder:
\begin{equation*}
  \C{\fun{in}}{n} = \C{\fun{pre}}{n} = 2n + 2.
  \index{in@$\C{\fun{in}}{n}$}
\end{equation*}
...
\(n\)~times. Clearly, this is the best case and
\begin{equation*}
  \B{\fun{in}_1}{n} = n + 1.
  \index{in1@$\B{\fun{in}_1}{n}$}
\end{equation*}
Thirdly, we should remark that a right rotation brings exactly one
more node (named~\(x\) in
...
% [0pt] of margin overhang
\centering
\includegraphics[bb=60 653 115 715]{zigzag} %721]
\caption{Zigzag}
\label{fig:zigzag}
\end{wrapfigure}
...
happens here when \(h = \lg(n+1)\), so
\begin{equation*}
  \B{\fun{bf}}{n} = 4n + 3\lg(n+1) + 3 \sim 4n.
  \index{bf@$\B{\fun{bf}}{n,h}$}
\end{equation*}
The maximum cost occurs when \(h=n\), so
\begin{equation*}
  \W{\fun{bf}}{n} = 7n + 3 \sim 7n.
\end{equation*}
We can now compare \fun{bf\(_0\)/1}\index{bf0@\fun{bf\(_0\)/1}} and
...
external node is~\(0\).) The definition of
\fun{per/1}\index{per@\fun{per/1}} (\emph{perfection}) is shown in
\fig~\vref{fig:per}.  If the tree~\(t\) is perfect, we also know its
height~\(h\): \(\fun{per}(t) \twoheadrightarrow \fun{true}(h)\).  Note
that the rules are ordered, so the last one may only apply when the
previous ones have not been matched. A refinement without inference
rules is shown in \fig~\vref{fig:per0}, where \(\fun{per}_0(t_1)\) is
evaluated before \(\fun{per}_0(t_2)\)\index{per0@\fun{per\(_0\)/1}}
since \(\fun{t}(\fun{per}_0(t_1), \fun{per}_0(t_2))\) is inefficient
if \(\fun{per}_0(t_1) \twoheadrightarrow \fun{false}()\).\index{binary
  tree!perfect $\sim$|)}
\begin{figure}
\centering
\includegraphics{per}
...
%\begin{figure}[b]
\centering
\includegraphics[bb=71 653 147 695]{complete} %721]
...
\begin{figure}
\centering
\includegraphics[bb=71 660 381 721]{comp}
...
\centering
\includegraphics[bb=71 653 153 720]{bal} %720]
\caption{}
\label{fig:bal}
\end{wrapfigure}
...
binary tree by a linear structure is called \emph{tree encoding}
\citep{Makinen_1991} and is related to the problem of generating all
binary trees of a given size \citep[7.2.1.6]{Knuth_2011}. One simple
approach consists in extending a traversal with the external nodes;
this way, enough information from the binary tree is retained in the
encoding, allowing us to unambiguously go back to the original tree.
...
\fig~\vref{fig:postorder} yields the following encoding: \([\Box,
  \Box, \Box, 0, 1, \Box, \Box, 2, 3, \Box, \Box, \Box, 4, 5, 6]\),
where \(\Box\) stands for \(\fun{ext}()\). Since a binary tree with
\(n\)~internal nodes has \(n+1\) external nodes (see
theorem~\ref{thm:int_ext} \vpageref{thm:int_ext}), the cost is
...
auxiliary stack. The definition of
\fun{post2b/1}\index{post2b@\fun{post2b/1}} (\emph{extended postorder
to binary tree}) is given in \fig~\vref{fig:post2b}.\index{binary
  tree!postorder!decoding}
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\begin{equation*}
  \C{\fun{post2b}}{n} = 2n + 3.
  \index{post2b@$\C{\fun{post2b}}{n}$}
\end{equation*}
The expected theorem is, of course,
\begin{equation}
...
\fig~\vref{fig:pre}. The cost is as simple as a postorder encoding:
\begin{equation*}
  \C{\fun{epre}}{n} = 2 n + 2.
  \index{epre@$\C{\fun{epre}}{n}$}
\end{equation*}
...
The definition is displayed in
\fig~\vref{fig:pre2b0}.
\begin{figure}
%\abovedisplayskip=0pt
%\belowdisplayskip=0pt
...
\begin{wrapfigure}[7]{r}[0pt]{0pt}
\centering
\includegraphics[bb=64 663 118 722]{binzip} %71 663 114 721
\caption{}
\label{fig:binzip}
\end{wrapfigure}
...
word; for example, chopping off the ends of \fun{rfrrff} yields
\fun{frrf}. Instead, we seek a decomposition of Dyck words into Dyck
...
This naturally yields the \emph{arch decomposition}\index{Dyck
  path!decomposition!arch $\sim$}, whose name stems from an
...
\label{fig:dpre}
\end{figure}
If the size of the binary tree is~\(n\), then the length of the Dyck path
is~\(2n\) and
\begin{equation*}
  \C{\fun{dpre}}{n} = 2n+2.
  \index{dpre@$\C{\fun{dpre}}{n}$}
\end{equation*}
...
and~\ref{fig:post2b}\index{post2b@\fun{post2b/1}} no
...
the transfer of some average parameters have been nicely presented by
