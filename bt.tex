...
\end{equation}
We used the catenation\index{stack!catenation} on stacks provided by
...
means that all the internal nodes being catenated come from the left
subtrees, the left subtree of the left subtree etc. so these nodes are
catenated again and again while going up (that is, returning from the
...
relying on catenation at all. This means that we want to build the
...
\begin{itemize*}
  ...
  associativity of stack catenation\index{stack!catenation}
...
\begin{itemize*}
  ...
  \item the cost of \(\fun{cat}(r,\fun{loc}_1(f'))\) is the length of
    the level~\(r\), plus one, thus the cumulative cost of catenation
...
Recall that \fun{rcat/2}\index{rcat@\fun{rcat/2}} (\emph{reverse and
  catenate}) is defined in equation~\eqref{def:rev}
...
root. Sometimes, updating a data structure with an on\hyp{}line
algorithm (see page~\pageref{par:online_vs_offline} and
section~\ref{sec:online} \vpageref{sec:online}) requires to keep a
direct access `inside' the data structure, usually where the last
update was performed, or nearby, in view of a better amortised (see
...
catenation of such words (whence the reference to prime factorisation
