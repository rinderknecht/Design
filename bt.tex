...
% [0pt] of margin overhang
\centering
\includegraphics[bb=62 653 115 728]{zigzag} %721]
\caption{}
\label{fig:zigzag}
\end{wrapfigure}
...
\(\fun{per}(t) \twoheadrightarrow \fun{true}(h)\).  Note that the
rules are ordered, so the last one may only apply when the previous
ones have not been matched. A refinement without inference rules is
shown in \fig~\vref{fig:per0}, where \(\fun{per}_0(t_1)\) is evaluated
before \(\fun{per}_0(t_2)\)\index{per0@\fun{per\(_0\)/1}} since
...
\begin{wrapfigure}[7]{r}[0pt]{0pt}
\centering
\includegraphics[bb=71 653 153 720]{bal}
\caption{}
\label{fig:bal}
\end{wrapfigure}
...
\centering
\includegraphics[bb=64 663 118 712]{binzip}
\caption{}
\label{fig:binzip}
\end{wrapfigure}
