...
\vpageref{eq:e_r}, proves \(1 + \floor{\lg n} =
\ceiling{\lg(n+1)}\). As a consequence, equation~\eqref{eq:tms_n_tmp}
can be rewritten as
\begin{equation}
...
x_0\) and positive if \(x > x_0\). Since \(p \geqslant 3\) implies
\(2^{p+1} > x_0\), the discriminant\index{discriminant|)} of \(K_p(i)
= 0\) is positive, which means that \(K_p(i)\) has three distinct real
roots if \(p \geqslant 3\), and so does \(\partial^2\Psi/\partial
i^2\). Otherwise, \(K_p(i)\) has one real root if \(0 \leqslant p
\leqslant 2\). Before we study these two cases in detail, we need a
small reminder about cubic polynomials. Let \(\rho_0\), \(\rho_1\)
and~\(\rho_2\) be the roots of \(P(x) = ax^3 + bx^2 + cx + d\). So
\(P(x) = a(x-\rho_0)(x-\rho_1)(x-\rho_2) = ax^3 -
a(\rho_0+\rho_1+\rho_2)x^2 + a(\rho_0\rho_1 + \rho_0\rho_2 + \rho_1
\rho_2)x - a(\rho_0\rho_1\rho_2)\), so \(\rho_0\rho_1\rho_2 = -d/a\).
\begin{itemize}
...
\begin{itemize}
  ...
  \item If \(0 \leqslant p \leqslant 2\), then \(\max_{0 < i
    \leqslant 2^p}\Psi(p,i) = \Psi(p,1)\). We easily check that
...
\fun{bms/1}\index{bms@\fun{bms/1}} and several auxiliaries. The call
\(\fun{solo}(s)\)\index{solo@\fun{solo/1}} is a stack containing
singletons with all the keys of~\(s\) in the same order. In other
words, it is the leaves of the merge tree. The call
\(\fun{all}(u)\)\index{all@\fun{all/1}} is a stack containing stacks
which are the result of merging adjacent stacks in~\(u\). In other
words, it is the level just above~\(u\) in the merge tree. The call
\(\fun{all}(\fun{solo}(s))\) is the sorted stack corresponding to the
stack of singletons~\(\fun{solo}(s)\). In other words, starting with
the leaves, it keeps building levels up by calling
\fun{mrg/2}\index{mrg@\fun{mrg/2}} until the root of the merge tree is
reached. What is beautiful about this program is that there is no need
for two distinct phases, first building the perfect merge trees and
then performing the unbalanced mergers with the roots of these: it is
...
computer algebra system, we find that \(\M{\fun{bms\(_0\)}}{n} <
\M{\fun{i2wb}}{n}\) if \(n \geqslant 43\), and \(\M{\fun{i2wb}}{n} <
\M{\fun{bms\(_0\)}}{n}\) if \(3 \leqslant n \leqslant 29\). For the
case \(n=2\), we find: \(\M{\fun{i2wb}}{2} = 11/2 > 5 =
...
least~\(43\) keys. In\hyp{}between, we don't know, but we can compute
efficiently the average costs and use dichotomy on the interval from
...
\section{On-line merge sort}
\label{sec:online}
\index{merge sort!on-line $\sim$|(}
...
and the latter are called \emph{on\hyp{}line}, as the sorting process
...
modified to become on\hyp{}line by remarking that balanced mergers
...
Our code is shown in \fig~\ref{fig:oms}\index{merge sort!on-line
  $\sim$!program}.
\begin{figure}
\begin{equation*}
\boxed{%
...
\end{array}}
\end{equation*}
\caption{On-line merge sort with \fun{oms/1}\label{fig:oms}}
\end{figure}
...
normal use case of on\hyp{}line merge sort, as, in practice, the
...
comparisons to sort \(n\)~keys on\hyp{}line. We have
\begin{equation}
...
%\addcontentsline{toc}{subsection}{Cost}
\paragraph{Minimum cost}
\index{merge sort!on-line $\sim$!minimum cost|(}
...
\paragraph{Maximum cost}
\index{merge sort!on-line $\sim$!maximum cost|(}
