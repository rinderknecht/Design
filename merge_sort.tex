...
\fun{tms}(\cons{x,y}{t}) & \rightarrow
                         & \cons{x}{\fun{tms}(\cons{y}{t})},\;
                           \text{if \(y \succcurlyeq x\)};\\
\fun{tms}(\cons{x,y}{t}) & \rightarrow
...
length of the original stack). The first rule of~\fun{tms/1} takes
advantage of natural, non\hyp{}decreasing runs of keys. In the second
rule, we saved one recursive call to \fun{cutr/3} and some memory by
calling \(\fun{cutr}([x],\cons{y}{t}, t)\) instead of
...
If the input is already sorted, we obtain the best case. To make the
analysis more interesting, let us ignore the first clause
of~\fun{tms/1} and have all comparisons performed by
\fun{mrg/2}\index{mrg@\fun{mrg/2}}. This new definition of
...
\centering
\includegraphics[bb=71 631 202 711]{msort_gen} % 721]
...
tight. We proved above that, if \(q=p-1\), that is, the binary
notation of~\(n\) starts with two 1-bits, f ormally written as
...
\OW{\fun{bms}}{2^p+i}.
\end{equation*}
This inequality is tight: \(\OW{\fun{tms}}{2^p+i} =
...
sequences of positive integers, that is, \(2^p+i > 2^{p-1}+j >
2^{p-2}+k > \dots\) and \(i > j > k > \dots\) The end of the latter
recursive descent is simply~\(0\), which means that the former stops
at a power of~\(2\), for which we know equation~\eqref{eq:2p}. In
other words, the binary representation of~\(n\) is made of a series of
one or more 1-bits (from \(2^p\), \(2^{p-1}\), \(2^{p-2}\), \ldots),
possibly followed by successive 0-bits, which we formally write
\(n=(1^+0^*)_2\), that is, \(n\)~is the difference between two powers
of~\(2\). Moreover, recalling that \(\OW{\fun{bms}}{2^q} =
\OW{\fun{tms}}{2^q}\), we have established
\begin{equation*}
\boxed{\OW{\fun{bms}}{n} = \OW{\fun{tms}}{n} \Leftrightarrow n=2^q \;
  \text{or} \; n=2^p - 2^q, \text{with \(p > q \geqslant 0\)}.}
\end{equation*}
...
\end{equation*}
where \(\alpha \simeq 1.2645\) and \(\ceiling{x} < x + 1\). Hence,
\begin{equation*}
...
\end{equation*}
implies \(\M{\fun{i2wb}}{n} < \M{\fun{bms\(_0\)}}{n}\). With the help
of a computer algebra system, we find that
\begin{enumerate}
